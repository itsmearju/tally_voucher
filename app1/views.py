...
def save_ledger(request):
    if 't_id' in request.session:
        ...
        if request.method=='POST':
            ...
            # # return render(request,'ledger_creditnote.html',{'tally':tally})
...
def save_receipt_details(request):
    if request.method=='POST':
        ...
        dest=request.POST['des']
        car=request.POST['carri']
        bills=request.POST['bill']
        date1=request.POST['dat_e']
...
def credit_party_list(request):
    if 't_id' in request.session:
        if request.session.has_key('t_id'):
            ...
            return redirect('/')
        tally = Companies.objects.filter(id=t_id)
    return render(request,'credit_party.html',{'tally':tally})
